    ...
    return (
      <Link href={`/venues/${venue.id}`}>
        <motion.div
          ...
          className="group relative glass neon-border rounded-2xl overflow-hidden cursor-pointer hover:border-[#ea6390]/40 transition-[background,box-shadow,border-color,opacity] duration-300 hover:shadow-[0_8px_40px_rgba(234,99,144,0.12)]"
        >
          {/* Image */}
          ...
          <div className="p-5 space-y-4">
            ...
            <span className="w-full flex items-center justify-between px-4 py-3 rounded-xl bg-gradient-to-r from-[#ea6390]/10 to-[#9e4280]/10 border border-[#ea6390]/15 text-[#ea6390] text-sm font-medium group-hover:border-[#ea6390]/30 group-hover:bg-[#ea6390]/15 transition-[background,border-color] duration-200">
  ...
  return (
    <Link href={`/venues/${venue.id}`}>
      <motion.div
        ...
        className="group relative glass neon-border rounded-2xl overflow-hidden cursor-pointer hover:border-[#ea6390]/40 transition-[background,box-shadow,border-color,opacity] duration-300 hover:shadow-[0_8px_32px_rgba(234,99,144,0.1)]"
      >
        {/* Image */}
        ...
        <div className="p-4 space-y-2.5">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className="text-sm font-bold text-[#ea6390] shrink-0">
              {venue.rating}
            </span>
          </div>
          ...
          <div className="flex flex-wrap gap-1">
            {venue.tags.slice(0, 2).map((tag) => (
              <span
                key={tag}
                className="px-2 py-0.5 text-xs rounded-full bg-white/5 border border-white/8 text-white/40"
              >
                {tag}
              </span>
            ))}
